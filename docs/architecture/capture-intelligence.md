...
The M4 baseline is photo-only. Milestone 6 adds a separate, optional Magic Search layer for current-project still-photo image/text embeddings and deterministic hybrid filters. It does not change the deterministic descriptor into a semantic embedding, merge semantic results into Similar Sets, or authorize People Brain, Moment Brain, video/audio intelligence, multicam synchronization, creative ranking, automatic culling, cloud work, model downloads, or a full local model manager. See [Magic Search architecture](magic-search.md).
